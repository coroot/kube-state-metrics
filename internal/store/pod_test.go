...
`,
	...
	for i, c := range cases {
		c.Func = generator.ComposeMetricGenFuncs(podMetricFamilies(c.AllowAnnotationsList, c.AllowLabelsList, 0))
		c.Headers = generator.ExtractMetricFamilyHeaders(podMetricFamilies(c.AllowAnnotationsList, c.AllowLabelsList, 0))
...
func BenchmarkPodStore(b *testing.B) {
	...
	f := generator.ComposeMetricGenFuncs(podMetricFamilies(nil, nil, 0))
